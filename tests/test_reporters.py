...
class TestDryRunReporter(unittest.TestCase):
    ...
    def test_build_report_with_all_match_types(self, mock_file: MagicMock) -> None:
        ...
        context = self._create_mock_context()
        context.terminated_matches = [
            TextMatch("Replaced", Path("f1.txt"), (0, 8), "test", "test", provider="rule:replace"),
            TextMatch("Skipped", Path("f1.txt"), (0, 7), "test", "test", provider="skipped"),
        ]
        context.cached_matches = [TextMatch("Cached", Path("f2.txt"), (0, 6), "test", "test", provider="cached")]
        context.matches_to_translate = [TextMatch("To Translate", Path("f3.txt"), (0, 12), "test", "test", provider="api")]
        self.reporter.generate(context)
        report_content = mock_file().write.call_args[0][0]
...
class TestSummaryReporter(unittest.TestCase):
    ...
    def test_generate_summary_logs_output(self, mock_log_info: MagicMock) -> None:
        ...
        context = ExecutionContext(task=self.task, config=self.config)
        context.terminated_matches = [
            TextMatch("A", Path("f.txt"), (0, 1), "test", "test", provider="rule"),
            TextMatch("B", Path("f.txt"), (0, 1), "test", "test", provider="skipped"),
        ]
        context.cached_matches = [TextMatch("C", Path("f.txt"), (0, 1), "test", "test", provider="cached")]
        context.matches_to_translate = [TextMatch("D", Path("f.txt"), (0, 1), "test", "test", provider="api", translated_text="d", tokens_used=10)]
        self.reporter.generate(context)
        calls = [
    ...
    def test_generate_summary_no_tokens(self, mock_log_info: MagicMock) -> None:
        ...
        context = ExecutionContext(task=self.task, config=self.config)
        context.matches_to_translate = [TextMatch("D", Path("f.txt"), (0, 1), "test", "test", provider="api", translated_text="d", tokens_used=0)]
        self.reporter.generate(context)
