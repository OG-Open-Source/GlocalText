...
class TestGeminiTranslatorInit(unittest.TestCase):
    def test_init_raises_value_error_if_api_key_is_missing(self):
        """5. Init Failure: Raises ValueError if API key is not provided."""
        # Arrange
        faulty_settings = ProviderSettings()  # No API key

        # Act & Assert
        with self.assertRaisesRegex(ValueError, "API key for Gemini is missing"):
